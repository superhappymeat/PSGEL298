#!/bin/bash
####################################################################################
#        Name: GEL.510.Define.Environment.sh                                       #
# Description: Ensure azure variables are set for the workshop for Track B         #
...
function logon-sp () {
    if  isFirstHost   ;     then
        ...
        "TF_VAR_client_id=${mySP_Id}" \
        "ARM_CLIENT_CERTIFICATE_PATH=/workspace/vars/GELENABLE_CREDS/SP_cert.pfx" \
        "TF_VAR_client_certificate_path=/workspace/vars/GELENABLE_CREDS/SP_cert.pfx" \
        "ARM_CLIENT_CERTIFICATE_PASSWORD=lnxsas" \
        "TF_VAR_client_certificate_password=lnxsas" \
